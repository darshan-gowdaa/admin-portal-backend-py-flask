...
function closeOpportunityModal() {
    ...
    // reset edit state so next open defaults to create mode
    currentEditId = null;
    ...
    fetch('/api/opportunities/')
        ...
        .then(data => {
            ...
            if (data && data.length > 0) {
                data.forEach(opp => appendCardToDOM(opp));
            } else if (grid) {
                grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-muted);">No opportunities found.</div>';
            }
        });
}

function appendCardToDOM(opp) {
    const grid = document.querySelector('.opportunities-grid');
    if (grid) {
        if (grid.innerHTML.includes('No opportunities found')) grid.innerHTML = '';
        const card = createCardElement(opp);
        grid.appendChild(card);
    }
}

function updateCardInDOM(opp) {
    const card = document.getElementById('opp-card-' + opp.id);
    if (card) {
        const newCard = createCardElement(opp);
...
function createCardElement(opp) {
    ...
    card.id = 'opp-card-' + opp.id;

    const skills = opp.skills_to_gain ? opp.skills_to_gain.split(',').map(s => s.trim()).filter(Boolean) : [];
    ...
    const footerHtml = `
        <div class="opportunity-footer">
            ...
            <div style="display:flex; gap:8px;">
                <button class="edit-btn" style="padding: 6px 12px; background: transparent; border: 1px solid var(--border-color); border-radius: 6px; cursor: pointer; color: var(--text-color);">Edit</button>
                <button class="delete-btn" style="padding: 6px 12px; background: #fee2e2; border: 1px solid #fecaca; border-radius: 6px; cursor: pointer; color: #ef4444;">Delete</button>
                <button class="view-course-btn" style="width: auto; padding: 6px 12px;">View Details</button>
            </div>
        </div>
    `;
    ...
    card.querySelector('.view-course-btn').addEventListener('click', function() {
        openOpportunityDetails(opp.name, {
            ...
            skills: skills,
            applicants: opp.max_applicants ? opp.max_applicants : 0,
    ...
    card.querySelector('.edit-btn').addEventListener('click', function() {
        ...
        // reverse-map backend value ("Data Science") back to HTML option value ("data")
        document.getElementById('oppCategory').value = CATEGORY_REVERSE_MAP[opp.category] || opp.category;
    ...
    card.querySelector('.delete-btn').addEventListener('click', function() {
        if (confirm('Are you sure you want to delete this opportunity?')) {
            fetch('/api/opportunities/' + opp.id, { method: 'DELETE' })
                .then(res => {
                    if (res.ok) {
                        // look up by id in case the DOM element was replaced by updateCardInDOM
                        const target = document.getElementById('opp-card-' + opp.id);
                        ...
                        res.json().then(d => showToast(d.error || 'Failed to delete opportunity'));
                    }
                });
        }
    });
...
// HTML dropdown value → backend string (used on submit)
const CATEGORY_MAP = {
...
// Backend string → HTML dropdown value (used when pre-filling edit form)
...
document.getElementById('opportunityForm').addEventListener('submit', function(e) {
    ...
    const rawCategory = document.getElementById('oppCategory').value;
    const payload = {
    ...
    if (!payload.name || !payload.duration || !payload.start_date || !payload.description || !payload.skills_to_gain || !payload.category || !payload.future_opportunities) {
        showToast('Please fill all required fields');
        return;
    }

    const isEditing = currentEditId !== null;
    const url = isEditing ? `/api/opportunities/${currentEditId}` : '/api/opportunities/';
